...
export default function ClearNumbersButton() {
  ...
  const handleClear = async () => {
    const confirmed = confirm('Tem certeza que deseja apagar TODOS os registros da tabela "numbers"? Esta ação é irreversível.');
    if (!confirmed) return;
    ...
    try {
      const res = await fetch('/api/config/delete/numbers?token=TCTelecom2025TC@', {
        method: 'DELETE',
      }
    );
      ...
      if (error instanceof Error) {
        setErrorMessage(
          error instanceof Error ? error.message : 'Erro inesperado ao apagar os registros.'
        );
      } else {
